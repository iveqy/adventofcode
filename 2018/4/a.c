...
int main()
{
	int current_guard = 0;
	int falls_asleep = 0;
	memset(guards, 0, 10000 * 60);
	for (int i = 0; i < LEN; i++) {
		int min = 0;
		char tmp[16];
		memset(tmp, 0, 16);
		strncpy(tmp, arr[i] + 15, 2);
		min = atoi(tmp);
		printf("str: %s\n", arr[i]);
		printf("min %d\n", min);
		if (min > 60)
			return -1;
		if (arr[i][19] == 'G') {
			char * at = strchr(arr[i], '#');
			strncpy(tmp, at + 1, strchr(at + 1, ' ') - at);
			current_guard = atoi(tmp);
			printf("Current_guard: %d\n", current_guard);
		} else if (arr[i][19] == 'w') {
			printf("Falls %d wakes %d\n",  falls_asleep, min);
			for (int i = falls_asleep; i < min; i++)
	...
	int tot = 0;
	for (int i = 0; i < 60; i++) {
		printf(" %d", guards[102][i]);
	}
	printf("\n");
	if (tot != 0)
		return 0;
	int max_tot = 0, max_guard = 0;
	...
	int max_min = 0;
	for (int i = 0; i < 60; i++) {
		if (guards[max_guard][i] > max_tot) {
			max_tot = guards[max_guard][i];
			max_min = i;
		}
	}
	for (int i = 0; i < 60; i++) {
		printf("%d ", guards[max_guard][i]);
	}
	printf("\n");
	printf("Maxguard: %d total: %d\n", max_guard, max_min);
	printf("%d\n", max_guard * max_min);
	return 0;
}
